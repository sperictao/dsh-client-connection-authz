...
/** Snapshot selected before authorization and request-body buffering. */
export interface SharedConnectionTarget {
...
export class HostConnectionService extends Service implements HostConnectionHandle {
   ...
   * @param requestGate - shared trust and plugin-authorization boundary.
   */
  ...
  /**
   * Select one shared-channel target before authorization and body buffering.
   * @param channel - shared channel mounted by Connection.
   * @param endpoint - validated channel-relative endpoint, when present.
   * @param fallback - handler for endpoints not claimed by the interceptor.
   * @param fallbackAuthority - authority required by the fallback endpoint.
   * @returns immutable handler/authority snapshot for this request.
   */
  selectSharedTarget(
    channel: '/api',
    endpoint: string | undefined,
    fallback: FetchHandler,
    fallbackAuthority: ConnectionRpcAuthority,
  ): SharedConnectionTarget {
    const interceptor = this.interceptors.get(channel)
    if (endpoint === undefined || interceptor === undefined || !interceptor.matches(endpoint)) {
      return { authority: fallbackAuthority, fetchHandler: fallback }
    }
    return {
      authority: interceptor.options.authority,
      fetchHandler: interceptor.fetchHandler,
    }
  }
  ...
  private register(
    ...
    const route: WebRoute = {
      ...
      handler: async (req, res) => {
        const endpoint = endpointFromPath(channel, new URL(req.url ?? '/', 'http://dsh.internal').pathname)
        const decision = this.requestGate.authorize(req, {
          transport: 'http',
          channel,
          ...(endpoint === undefined ? {} : { endpoint }),
          requiredAuthority: options.authority,
        })
        if (!decision.allowed) {
