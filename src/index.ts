...
/** Required providers; API Proxy remains an optional `/api` fallback. */
 ...
 * non-loopback requests additionally pass through the injected authorizer.
...
export function apply(ctx: Context, config?: ConnectionConfig): void {
  ...
  const requestGate = new ConnectionRequestGate(trustedHosts, ctx.connectionRequestAuthorizer)
  const connection = new HostConnectionService(ctx, requestGate)
  const fallbackFetchHandler: FetchHandler = {
    async fetch(request) {
  ...
  const route: WebRoute = {
    ...
    handler: async (req, res) => {
      ...
      const fallbackAuthority = endpoint !== undefined && PRIVILEGED_METHODS.has(endpoint)
        ? 'loopback'
        : 'trusted-host'
      const target = connection.selectSharedTarget(
        API_PATH,
        endpoint,
        fallbackFetchHandler,
        fallbackAuthority,
      )
      const decision = requestGate.authorize(req, {
        transport: 'http',
        channel: API_PATH,
        ...(endpoint === undefined ? {} : { endpoint }),
        requiredAuthority: target.authority,
      })
      if (!decision.allowed) {
        denyHttpRequest(res, decision)
        return
      }
      await bridge(req, res, target.fetchHandler, maxRequestBodyBytes)
    },
  }
    ...
    const registerDownlink = (
      ...
      apiCtx.effect(() => apiCtx.webServer.registerUpgrade({
        ...
        handler: (req, socket, head) => {
          ...
          if (!decision.allowed) {
            rejectWebSocketUpgrade(socket, decision.status)
            return
          }
