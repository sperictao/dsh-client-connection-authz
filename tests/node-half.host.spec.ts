...
const trustedHostAuthorizer: ConnectionRequestAuthorizer = {
  authorize(facts) {
    if (facts.requiredAuthority === 'loopback') return { allowed: false, status: 403 }
    return {
      allowed: true,
      principal: { provider: 'test', subject: 'remote@example.com' },
    }
  },
}

function provideAuthorizer(ctx: Context): void {
  ctx.provide('connectionRequestAuthorizer', trustedHostAuthorizer)
}
